...
function AddBudgetModal({ isOpen, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    const isDuplicate = budgets.some((budget) => budget.category === formData.category && budget.period === formData.period); // Implement duplicate check if needed
    if (isDuplicate) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="transaction-form">
          ...
          <div className="form-group">
            ...
            <input type="number" id="limit" name="limit" value={formData.limit} onChange={handleChange} placeholder="0.00" step="0.01" required />
          </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => setIsDuplicateDialogOpen(false)} // Klik OK -> Tutup dialog peringatan
        onCancel={() => setIsDuplicateDialogOpen(false)}
        showCancel={false} // Klik Cancel -> Tutup dialog peringatan
      />
    </div>
  );
}
