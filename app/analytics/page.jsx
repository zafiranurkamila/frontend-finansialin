...
export default function AnalyticsPage() {
  ...
  const categoryData = transactions.reduce((acc, transaction) => {
    ...
    const categoryName = transaction.category?.name || getCategoryById(transaction.idCategory)?.name || "Uncategorized";
                ...
                <div className="charts-grid">
                  ...
                  <div className="chart-card wide">
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <PieChart>
                        <Pie data={pieData} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} outerRadius={140} fill="#8884d8" dataKey="value">
                            ...
                            return (
                              <div key={budget.id || budget.idBudget} className="budget-alert-item">
                                <div className="budget-alert-content">
                                  <p className="budget-alert-category">{budget.category} - Over Budget</p>
                                  <p className="budget-alert-details">
                                    Spent: Rp{progress.spent.toLocaleString("id-ID")} / Limit: Rp{budget.limit.toLocaleString("id-ID")}{" "}
                                    <span style={{ color: "#EF4444", fontWeight: 700 }}>(Over by Rp{progress.remaining.toLocaleString("id-ID")})</span>
                                  </p>
                                </div>
                                <div className="budget-alert-progress">
                                  <div className="budget-progress-bar">
                                    <div className="budget-progress-fill" style={{ width: `${Math.min(progress.percentage, 100)}%` }}></div>
                                  </div>
                                  <p className="budget-percent">{progress.percentage.toFixed(0)}%</p>
                                </div>
                                <div className="budget-alert-action">
                                  <button className="budget-alert-btn" onClick={() => router.push(`/budget?highlighted=${budget.id || budget.idBudget}`)}>
                                    View Budget
                                  </button>
                                </div>
                              </div>
